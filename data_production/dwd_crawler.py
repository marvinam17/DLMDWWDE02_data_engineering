...
def rename_dataframe_columns(df) -> DataFrame:
    """
...
def reformat_timestamp(df):
    df["MEASUREMENT_DATE"] = pd.to_datetime(df["MEASUREMENT_DATE"], format="%Y%m%d%H%M")
    return df


def get_and_unzip_files(url, list_of_filenames) -> DataFrame:
    """
    ...
    return (
        ...
        .drop_duplicates(subset=["MEASUREMENT_DATE"])
    )


def get_station_dwd_file_storage(url, station_id) -> list:
    """
