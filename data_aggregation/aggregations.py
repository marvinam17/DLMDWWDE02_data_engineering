...
def remove_invalid_rows_multi(df: DataFrame, column: list) -> DataFrame:
    ...
    >>> from pyspark.sql import SparkSession
    >>> spark = SparkSession.builder.master("local[2]").appName("Test").getOrCreate()
    >>> df = spark.createDataFrame([(-999.0,20.0), (20.0,30.0), (20.0,-999.0)],["example_column","second_column"])
...
def apply_aggs(
    df: DataFrame, aggs: list, clean_column: list, group_columns: list
) -> DataFrame:
    """
    ...
    >>> import pyspark.sql.functions as F
    >>> spark = SparkSession.builder.master("local[2]").appName("Test").getOrCreate()
    >>> df = spark.createDataFrame([("1999-02-01",-999.0,20.0,), ("1999-02-01", 20.0,30.0,),("1999-02-01", 20.0,-999.0,), ("1999-02-01", 30.0,20.0,)],["measurement_date","example_column","second_column"])
