...
def apply_schema(df: DataFrame, schema:StructType) -> DataFrame:
    ...
    >>> spark = SparkSession.builder.master("local[2]").appName("Test").getOrCreate()
    >>> SCHEMA = StructType([StructField("measurement_date", StringType()),
    StructField("station_id", StringType()),
    StructField("temperature", FloatType()),
    StructField("quality", IntegerType())])
    >>> df = spark.createDataFrame([(b'{"measurement_date":"199902011511",
    "station_id":"00433","temperature":20.0,"quality":1}',)], ["value"])
...
def join_static_data(df: DataFrame, df_static: DataFrame) -> DataFrame:
    ...
    >>> df_static = spark.createDataFrame([("00433","ETNN","example",50), 
    ("00599","ETNL","example_2",52)], 
